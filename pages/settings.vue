...
// Close all positions (without deactivating account)
...
// Market search for asset dropdown
...
const searchMarkets = async () => {
  if (!selectedAccount.value || !marketSearchTerm.value || marketSearchTerm.value.length < 2) {
  ...
  try {
    const result = await $fetch<{ markets: MarketInfo[] }>(
      `/api/settings/${selectedAccount.value}/markets`,
...
const executeBulkImport = async () => {
  if (!selectedAccount.value || !bulkImportJson.value) return;

  bulkImporting.value = true;
  bulkImportError.value = null;

  try {
    const assets = JSON.parse(bulkImportJson.value);
    ...
    const result = await $fetch<{
      ...
      total: number;
      errors?: string[];
    }>(`/api/settings/${selectedAccount.value}/assets/bulk`, {
      method: "POST",
      body: {
        assets,
    ...
    <div class="max-w-7xl mx-auto space-y-6">
      ...
      <template v-else-if="editingInfo">
        ...
        <div class="space-y-4">
          ...
          <!-- Bulk Import Modal -->
          <UModal v-model:open="showBulkImportModal">
            <template #content>
              <UCard class="w-full max-w-2xl">
                <template #header>
                  <div class="flex items-center gap-2">
                ...
                <template #footer>
                  <div class="flex justify-end gap-2">
                    <UButton
                      variant="ghost"
                      @click="showBulkImportModal = false; bulkImportJson = ''; bulkImportError = null"
                    >
                      Abbrechen
                    </UButton>
                    <UButton
          ...
          <UCard v-if="showNewAssetForm">
            ...
            <div class="space-y-4">
              ...
              <div class="flex gap-2">
                ...
                <UButton variant="ghost" @click="showNewAssetForm = false; newAssetName = ''; marketSearchTerm = ''; availableMarkets = []">
                  Abbrechen
                </UButton>
              </div>
            </div>
